...
def check_update():
    ...
    guidance = ''
    if has_update:
        guidance = (f'发现新版本 v{latest}（当前 {installed}）。请手动升级：'
                    f'在 DSH 安装目录执行 npm install @deepseek-ai/dsh@latest，然后重启 DSH。')

    return {'installed': installed, 'latest': latest, 'has_update': has_update,
