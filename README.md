...
- [x] **[中] DSH 检查更新 → 升级指引（移除一键更新 / 重新安装）**：设置页移除「一键更新 / 重新安装」按钮；「检查更新」发现新版本时提示版本差异（当前 vs 最新）并给出升级操作指引——开发者：`cd <DSH 安装目录> && npm install @deepseek-ai/dsh@latest` 后重启 DSH；保持「关联已有 DSH」与 DSH 独立升级原则（PLW 不接管 DSH 安装文件，互不耦合）；同步修订 `doc/README-架构边界与升级指南.md` §3.2 表格
