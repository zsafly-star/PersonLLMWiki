...
详见 [架构与集成](doc/01-架构/01-架构与集成.md) 第三部分「架构收敛待办」（应用内待办同步记录）。

- [X] **[高] MCP Client 总线**（`mcp_client.py` / `/api/mcp/servers`）标记 legacy，1.1 评估移除
- [X] **[中] SAP 定时同步改走 DSH headless**（或直连 HTTP/DB），移除 PLW 主动调 SAP MCP 的依赖（与上一条一起评估）
- [X] **[中] PLW 内置对话 agent 工具源限缩**到自身 19 个 MCP 工具，外部能力一律由 DSH 承接

### 模块完善（1.1：已回归）

- [X] **[中] 共享中心** — 发布 / 安装流程打磨后重新开放菜单
- [X] **[中] 笔记** — 功能完善后重新开放菜单
...
- [X] **[中] DSH 集成收敛**：`shared/routes.py` 的 `_append_cordis_patch` 与 DSH 实际 cordis 语法（数组 `insert:`）/数据目录（`~/.dsh`）不一致，统一重构为 `get_dsh_data_home()` + 标准 insert 语法（「检查更新 → 升级指引」前置）
- [X] **[中] DSH 检查更新 → 升级指引（移除一键更新 / 重新安装）**：设置页移除「一键更新 / 重新安装」按钮；「检查更新」发现新版本时提示版本差异（当前 vs 最新）并给出升级操作指引——开发者：`cd <DSH 安装目录> && npm install @deepseek-ai/dsh@latest` 后重启 DSH；保持「关联已有 DSH」与 DSH 独立升级原则（PLW 不接管 DSH 安装文件，互不耦合）；同步修订 `doc/01-架构/01-架构与集成.md` 第一部分 §3.2 表格
...
**收尾优化（T13 完成后继续，归属 1.2.1）**

- [X] **[中] 删除「笔记」功能**：`modules/note/` 空壳（routes.py 仅渲染模板，无后端/模型/API）+ 侧边栏 `/note` 菜单 + `note_bp` 注册 + `article.js`/`editor.js` 遗留 note 死代码一并清理；**保留** MCP `read_note`/`write_note`（文章工具，命名巧合）与 `resource/article/个人笔记/`（用户数据）。
- [X] **[中] 共享中心 UI 优化**：按 T13 记忆管理页 / 技能候选 UI 的卡片规范统一打磨（复用 `.am-mcp-card` + 设计令牌 + `/ui-ux-pro-max` 技能）。
- [X] **[高] 打包前 5 轮代码优化**：发版前执行 5 轮「评审 → 修复 → 回归」代码优化循环（死代码清理、命名统一、性能与可维护性），以最终验证通过的构建产物为准。
- [X] **[低] 统一硬编码版本号**：`src/app.py` 的 `/api` 接口返回 `version: '1.0.0'`（且 `name: 'SSEditor'` 旧品牌）+ `src/modules/mcp/routes.py` 的 `MCP_SERVER_VERSION = '1.0.0'` 均硬编码、未随 1.2.0 更新；统一改为 `1.2.0` / `PersonLLMWiki`；`shared.html` 的 `placeholder="1.0.0"` 一并更新。
- [X] **[高] 安装版静默升级未生效**：实测「下载并升级」后 `D:\PersonLLMWiki\_internal\app_version.txt` 仍为 `1.1.0.001`、exe 时间戳仍为旧构建（08-27），即静默安装未真正替换应用（`C:\Program Files\PersonLLMWiki` 也不存在）。疑似：`/VERYSILENT` + `installer.iss` `PrivilegesRequired=admin` 经 `wscript.exe` 拉起时 UAC 未通过 / `_WAIT_BEFORE_INSTALL_MS=3000` 等待不足致 exe 被占用替换失败。需排查静默安装链路（watcher VBS / UAC 提权 / 退出时序），并考虑 `build_silent_args` 显式加 `/DIR` 指向当前安装目录。
- [X] **[中] 记忆 SKILL 内联/收尾复盘未自动触发**（已落地：DSH 侧「收尾复盘契约」= 记忆 SKILL 三段式必做 + 收束信号触发 + 逐条查重去重 + 接入批量沉降通道；PLW 侧「多信号沉降」= 空闲超时/批量沉降通道 / `settle_memory`）：实测 DSH 对话中只有用户明确「记住」才写入记忆，SKILL 的「内联记录」「收尾复盘」未自动触发（纯靠 agent 自觉 remember 不可靠）。需 DSH 侧会话结束钩子/workflow 触发复盘，或落地「多信号沉降」的空闲超时/批量沉降通道。
- [X] **[中] 托盘退出未杀 DSH 进程**：PLW 桌面壳托盘「退出」时，DSH（3080）进程未一并终止、残留占用。需在退出逻辑里复用 `dsh_bridge` 停止逻辑一并清理 DSH（注意只停 PLW 拉起的 DSH，不误杀用户自启的 DSH）。
...
| **1.1**   | 三层架构落地 + DSH 交互收敛 + 模块回归    | 架构收敛（MCP Client 总线 legacy / SAP 改 headless / 对话工具源限缩）、DSH 集成收敛、DSH 检查更新→升级指引、共享中心 / 笔记回归                                 | 已完成 |
| **1.2.0** | 记忆与上下文（OpenViking 替代） | 记忆模块（过程级采集 + 原始 trace + decision 入图 + 记忆管理页）、分层上下文交付、技能沉淀、记忆/知识双轨制 | 已完成 |
| **1.2.1** | 收尾优化 | 删除笔记功能、共享中心 UI 优化、打包前 5 轮代码优化、统一硬编码版本号、安装版静默升级修复、记忆 SKILL 自动触发（DSH 契约 + 多信号沉降）、托盘退出杀 DSH、版本号 1.2.0→1.2.1 | 已完成 |
...
- 发版以最终验证通过的构建产物为准，每轮验证走 [打包验证任务书](doc/03-分发打包/02-打包验证任务书.md)；
...
**与 DeepSeek Harness（DSH）的分工**：PLW 管「内容与公司能力」（知识编译/检索/审批/工具），DSH 管「执行编排」（goal/workflow/subagent/skills）。二者通过 MCP 标准协议协作，详见 [架构与集成](doc/01-架构/01-架构与集成.md)。
...
- [架构与集成](doc/01-架构/01-架构与集成.md) — 系统总览、PLW 与 DSH 分工、升级方法、集成架构、共享中心、里程碑（**入口文档**）
- [PersonLLMWiki 设计规范](doc/02-设计/01-PersonLLMWiki设计规范.md) — 各子系统设计索引
- [记忆与上下文交付设计方案](doc/02-设计/05-记忆与上下文交付设计方案.md) — 会话记忆 / 分层交付 / 技能沉淀（OpenViking 替代，1.2）
- [打包验证任务书](doc/03-分发打包/02-打包验证任务书.md) — 发版打包验证 SOP（可被任意 AI 工具执行）
- [打包与发布](doc/03-分发打包/01-打包与发布.md) — 打包、分发与发布流程（zip + Embedded Python）+ 安装版自动升级
...
| 类别               | 技术                                                                                                       |
| ------------------ | ---------------------------------------------------------------------------------------------------------- |
| **后端**     | Flask（Flask-SQLAlchemy / Flask-CORS）, APScheduler, SQLite                                                                     |
...
| **检索**     | Embedding API + fastembed（本地 ONNX）+ BM25（rank_bm25 + jieba 分词）                                                      |
| **文档解析** | PyMuPDF（PDF MCP）+ Python-Markdown（Markdown 渲染）+ Pillow（图片处理）                                                                                         |
...
- [jieba](https://github.com/fxsjy/jieba)、[rank_bm25](https://github.com/dorianbrown/rank_bm25)、[fastembed](https://github.com/qdrant/fastembed)（中文分词、BM25 检索、本地向量化）
- [PyMuPDF](https://pymupdf.readthedocs.io/)、[Python-Markdown](https://python-markdown.github.io/)、[Pillow](https://python-pillow.org/)（PDF 解析、Markdown 渲染、图片处理）
- [D3.js](https://d3js.org/)（知识星链可视化）
- [llm-wiki-compiler](https://github.com/atomicmemory/llm-wiki-compiler)（LLM Wiki 模式启发）
