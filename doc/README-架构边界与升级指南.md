...
│  wiki 编译/检索/审批 · 文章图片 · 定时任务 · MCP Server 19 工具│
└──────────────────────────────────────────────────────────┘
```
...
- MCP 工具面（`mcp__personllmwiki__*`）由 PLW 侧 19 工具注册表决定，DSH 升级不改变它们。
...
|---|---|---|
| **本文** | 入口总览 | 边界 + 升级 + 导航 |
| DSH集成架构设计方案.md | 架构细节（v0.6） | 三件套、知识供给、共享中心、里程碑、决策清单 |
| PersonLLMWiki设计规范.md | PLW 设计索引 | 各模块设计文档入口（原 DESIGN.md，已按品牌重命名） |
| 打包验证任务书.md | 发版 SOP | 构建/校验/冒烟/人工安装验证清单（可被任意 AI 工具执行） |
| 分发部署方案.md | PLW 打包分发 | Embedded Python / 安装包 / 升级.bat |
| 开发者打包发布指南.md | 发布流程 | 打包、Release 发布 |
| 用户使用指南.md | 用户手册 | 桌面端 / Web 使用说明 |
| ZSSNote_MCP_设计方案.md | MCP 深度设计 | 19 工具、双角色、内置服务（品牌名已标注更新） |
