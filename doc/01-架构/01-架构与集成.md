# PersonLLMWiki × DeepSeek Harness 架构与集成

> 本文合并自「架构边界与升级指南」（入口总览）、「DSH 集成架构设计方案」（集成细节）与「待办：架构收敛」（已完成的收敛项）。
> 版本：边界/升级 v1.0（2026-08-21）｜ 集成 v0.6（2026-08-24）｜ 收敛待办已随 1.1.0 完成。
> 关联文档：[PersonLLMWiki设计规范](../02-设计/01-PersonLLMWiki设计规范.md)（子系统索引）、[打包与发布](../03-分发打包/01-打包与发布.md)、[MCP设计方案](../02-设计/02-MCP设计方案.md)、[打包验证任务书](../03-分发打包/02-打包验证任务书.md)。
...
│  wiki 编译/检索/审批 · 文章图片 · 定时任务 · MCP Server 24 工具│
└──────────────────────────────────────────────────────────┘
```

- **PLW 管"内容与企业能力"**：知识 + 记忆的生产、编译、检索、审批、SAP 等企业系统、定时调度（1.2 起记忆模块落 PLW，DSH 经 MCP 消费）；
- **DSH 管"干活"**：目标执行、多步编排、技能、记忆（会话级上下文）——通过 MCP 消费 PLW 的能力。
...
- MCP 工具面（`mcp__personllmwiki__*`）由 PLW 侧 24 工具注册表决定，DSH 升级不改变它们。
...
| v0.6 | 2026-08-24 | C1 有损裁剪：officecli 退役（MCP 工具 28→19）；共享中心一期（浏览/详情/安装）与二期（发布）完成；桌面壳 shell-bar 并入 SPA 顶栏（单栏）；DSH 启动非阻塞 `starting` 态 + `--no-open`；新增《02-打包验证任务书.md》。内部文档（多智能体 PRD/技术方案、变更实施说明、bug-fix）已随开源化移除 |
...
│  · MCP Server：24 个工具（search_kb / read_wiki_page /       │
...
│  · Tab 壳：工作台/知识库/文章/…/【智能体=DSH iframe】          │
...
| 设施 | 位置 | 状态 |
|---|---|---|
| MCP Server 24 工具（Tier1 只读 7 / Tier2 检索 1 / Tier3 写入 7 / Todo 1 / Workspace 3；Office 9 已随 C1 有损裁剪退役） | `src/modules/mcp/tools_registration.py` | ✅ 已注册，`/mcp` 对外暴露 |
| 混合检索（向量 0.7 + BM25 0.3，jieba 分词，内存常驻） | `src/modules/wiki/compiler/retrieval.py` | ✅ `hybrid_search()` |
| 知识供给（检索 + 循环中按需） | `src/modules/wiki/compiler/retrieval.py` + `search_kb` handler | ✅ 同一 `search_kb` handler，对话页已移除，改由 DSH agent 经 `/mcp` 调用 |
...
| 公共实例部署文档 | 复用《打包与发布》思路，补充 Linux 部署 + 常驻进程（systemd） | P1 |
...
4. 消息源待办自动执行场景（见 §13）是"效率功能"的职责延伸：todo=任务队列、automation=调度器、DSH=headless 执行器。
...
- PersonLLMWiki 侧**零代码改动**，`search_kb` 等 24 个工具原样暴露；
...
- 对话页已移除（T14），知识供给统一由 DSH agent 经 `/mcp` 调 `search_kb` / `read_wiki_page`（预检索或循环中按需，由 DSH 侧 SKILL / prompt 引导）。

#### 6.2 DSH 接入三层次

| 层次 | 做法 | 工作量 | 说明 |
|---|---|---|---|
| **L1（推荐）** | DSH MCP 客户端配置指向 `/mcp` | 配置级 | agent 会话内自动出现 `personllmwiki__search_kb` 等工具，行为与循环中按需检索一致 |
| **L2（可选）** | DSH profile 提示词 / SKILL 引导「必先检索」 | 配置级 | 复刻预检索（eager）语义 |
...
| 定时自动化（APScheduler） | **保留薄壳**：表结构与运行记录不变；执行统一经 `dsh_bridge` 调 `dsh --profile headless "prompt"`。**已验证 V2：dsh-schedule 是会话内提醒（无 cron 表达式、冷会话不执行），不能承担无人值守定时，薄壳结论不变** |
...
| headless 执行 | 已实现 | `dsh_bridge.run_headless` |
...
> PLW 只保留知识库 MCP Server + 调度 + 桌面壳；PLW 的 MCP Client 总线为**过渡态**（1.1 已标记 legacy，本次不拆）。
> 对应应用内待办 id 4/5/6（本文件为仓库内持久记录，防应用数据丢失后无从追溯）。

> ✅ **已于 1.1.0 完成**：三项待办均已落地——MCP Client 总线标记 legacy（保留给内置 subprocess + MCP 管理 UI，评估后续移除）、SAP 定时同步改直连 SQLite、对话 agent 工具源限缩到自身 19 个。完成标准见文末勾选。
...
- [x] PLW 不再主动连接任何外部 MCP Server（SAP 同步已改直连 SQLite，对话 agent 已限缩到自身工具；MCP Client 总线标记 legacy）；
- [x] 调度器 SAP 同步改为 DSH headless 或直连方案，功能等价；
- [x] 对话页 agent 仅使用 PLW 自身工具，外部工具经 DSH；
- [x] README / 架构图 / 本文件同步更新，`mcp_client.py` 相关代码移除或标注 legacy。
