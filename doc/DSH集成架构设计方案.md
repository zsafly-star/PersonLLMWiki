...
> 版本：v0.6（2026-08-24）｜ 状态：核心功能已实现，文档随实现演进 ｜ 日期：2026-08-20
> 关联文档：《PersonLLMWiki设计规范.md》《分发部署方案.md》《ZSSNote_MCP_设计方案.md》《打包验证任务书.md》
...
| v0.3 | 2026-08-21 | ①PLW 侧边栏「知识组 / 效率组」分组（§4.5）；②消息源待办 → 智能体自动执行场景纳入规划（§13）；③效率组形态定案：不拆应用、不做 DSH 插件，能力经 MCP 工具互通（D10/D11） |
...
| v0.6 | 2026-08-24 | C1 有损裁剪：officecli 退役（MCP 工具 28→19）；共享中心一期（浏览/详情/安装）与二期（发布）完成；桌面壳 shell-bar 并入 SPA 顶栏（单栏）；DSH 启动非阻塞 `starting` 态 + `--no-open`；新增《打包验证任务书.md》。内部文档（多智能体 PRD/技术方案、变更实施说明、bug-fix）已随开源化移除 |
...
| 设施 | 位置 | 状态 |
|---|---|---|
| MCP Server 19 工具（Tier1 只读 7 / Tier2 检索 1 / Tier3 写入 7 / Todo 1 / Workspace 3；Office 9 已随 C1 有损裁剪退役） | `src/modules/mcp/tools_registration.py` | ✅ 已注册，`/mcp` 对外暴露 |
...
| 检查更新 | 已装版本 vs npm 最新 → 显示版本差异 + 升级指引（开发者手动 `npm install @deepseek-ai/dsh@latest`）→ 重启 DSH | 1.1 起 DSH 独立手动升级，PLW 不接管 DSH 安装文件 |
| 安装 PLW 知识库插件 | 技能落到 `$DSH_HOME/skills/`，MCP 连接写入 cordis.patch.yml | 一键接入知识库 |
...
**演进记录**：v0.3 曾定案「侧边栏知识组/效率组分组」并实现（`826cbd2`），但用户反馈：①菜单栏出现分组文字不符合预期；②「品牌 + Wiki|DSH 开关」在浏览器入口看不到（根因：开关只存在于桌面壳 `/shell`）。→ **v0.4 撤销分组，改为统一入口**（`/` → shell 壳页）。v0.6 进一步将 shell-bar 并入 SPA 顶栏（单栏），开关/状态点移入 SPA 统一顶部，DSH 模式以悬浮按钮返回。
...
4. 消息源待办自动执行场景（§13）是"效率功能"的职责延伸：todo=任务队列、automation=调度器、DSH=headless 执行器。
...
| V5 | 插件分发渠道 | ✅ 已解决：私有 npm 制品库（hosted 托管插件包 + proxy 代理 npmjs），git 分发作回退 | 插件走私有制品库，`dsh plugin add` 接入 |
...
| **P2 桌面端 Plan B**（~2 周） | 模式切换壳（Wiki\|DSH 顶栏开关 + 单 iframe 全屏）+ dsh_bridge + 设置页 DSH 管理（关联/检查更新）+ 侧边栏分组（§4.5） | 桌面端一个窗口内完成知识库+智能体（全模式切换），DSH 缺失时优雅降级 |
| **P3 共享 + 插件**（~2 周） | 共享中心一期（git + wiki 目录 + agent.json）+ DSH 插件 L1 | 同事可发布/安装共享 agent；`dsh plugin add` 一条命令接入知识库 |
| **P4 消息源待办自动化**（~2 周） | 消息平台会话存档 MCP 服务 + todo 队列扩展 + 轮询领取 + DSH headless 执行（§13） | 每天自动拉取消息待办入库；待办被智能体自动认领执行并回写状态 |

> 实现快照（2026-08-20）：P2 的「关联已有 DSH / 启动 / 停止 / 更新检查」已落地。**1.1.0 修订**：移除「重新安装（下载运行时）/ 一键更新（换 app 留 home）」，DSH 改为独立手动升级（检查更新仅显示版本差异 + `npm install` 指引）；「token 替换文档化」仍归入后续里程碑。
...
| D5 | DSH 更新源 | 私有制品库 zip / npm registry / 并存 | **并存**：npm registry（proxy npmjs + hosted 插件）管 npm 包，私有制品库管运行时 zip |
| D6 | 便携 Node 捆绑 | 捆绑（同事可用）/ 不捆绑（仅开发者） | **捆绑作为可选兜底**：无 Node 环境走重新安装；开发者走「关联已有 DSH」 |
...
| D11 | 消息源待办自动化 | 纳入规划（§13） | **纳入 P4**：前提=消息平台会话存档可开通；执行分级（默认只读自动，写操作白名单/确认） |

---

## 13. 场景：消息源待办 → 智能体自动执行（P4）

> 本场景为**可选企业集成示例**：将 IM/消息平台（如企业微信/钉钉/飞书等，需支持会话存档类 API）中的待办自动沉淀为任务队列，再由智能体认领执行。

### 13.1 场景描述

每天定时从消息平台聊天记录拉取待办 → 写入 PLW todo（任务队列）→ 智能体轮询监控，发现符合自身能力的待办自动认领执行 → 结果回写。
...
DSH agent 执行（经 MCP 调 search_kb / write_note / 企业系统 API…）
...
| 组件 | 类型 | 说明 |
|---|---|---|
| 消息平台会话存档 MCP 服务 | **新增**（service.json subprocess，仿 pdf-mcp/websearch 自包含模式） | 拉取+解密+消息解析；本身可共享（呼应共享中心） |
| 提取待办 automation 任务 | 复用现有 | prompt 描述提取规则；工具范围=消息服务 + `create_todo` |
...
| 执行状态记录 | **轻量复用** tasks 模块 `state_store` | 只取 L4 状态表（任务执行进度/结果/失败重试点），**不搬 orchestrator/router** |
| headless 执行 | 已实现 | `dsh_bridge.run_headless` + 回退 react loop |

### 13.4 关键约束

1. **会话存档有开通门槛**：需平台管理员开通 + 合规审批 + 可信 IP + 密钥（平台官方解密 SDK）；聊天记录拉取需遵循当地数据保护法规。拿不到存档 API 时，退化为应用消息回调（范围窄）；
2. **"实时"= 分钟级**：存档 API 是拉取模式，轮询 1~5 分钟是实际可达值；
3. **headless 无会话连续性**：多步任务依赖 §13.3 的执行状态表承载上下文与断点；
4. **自动执行分级**：默认只放行只读/低风险（检索、整理、生成草稿）；写操作（write_note、企业系统写接口）进白名单或执行前推送确认。

### 13.5 验收标准（P4）

- 每天定时从消息平台拉取消息并提取待办入库（来源标注 `msg`）；
